...
export default function Ai4Q1M() {
  const router = useRouter();
  const [progress, setProgress] = useState(0);
  const options = ["IELTS", "TOEFL", "PTE", "Duolingo"];
    ...
    <main className="min-h-screen bg-[#DFFFFF] text-[#545454] flex flex-col">
      ...
      <div className="py-15 bg-[#DFFFFF]">
        <div className="px-5 pb-2">
          {/* standard header: back button + centered title */}
      ...
      <section className="flex-1 w-full bg-white rounded-t-3xl shadow-lg px-5 pt-6 pb-12">
        <div className="max-w-full mx-auto">
          <div className="px-1 mb-4">
            <h2 className="text-2xl font-semibold mb-3">Choose English Test</h2>
            <p className="text-lg font-medium mb-6">
              Which English Test Have You Taken Or Planning To Take?
            </p>
          </div>

          <form className="space-y-3" aria-label="Choose English test">
            {options.map((opt, idx) => {

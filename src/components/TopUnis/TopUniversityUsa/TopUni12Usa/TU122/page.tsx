"use client";

import Image from "next/image";
import React from "react";

const points = [
  {
    text: "Ranked #26 in QS World University Rankings 2025.",
    style: "absolute left-[-16%] top-[25%] w-[30%] text-right",
  },
  {
    text: "Around 23,000 students, including 20% international students from 100+ countries.",
    style: "absolute left-[15%] top-[-7%] w-[36%] text-center",
  },
  {
    text: "Campuses in Evanston, Chicago, and Qatar, offering a truly global education.",
    style: "absolute left-[54%] top-[20%] w-[36%] text-left",
  },
  {
    text: "  #14 globally for graduate employability (QS 2025).",
    style: "absolute left-[40%] top-[80%] w-[26%] text-center",
  },
  {
    text: "Home to 22 Nobel laureates, 40+ Pulitzer Prize winners, and leading innovators.",
    style: "absolute right-[-11%] top-[63%] w-[36%] text-left",
  },
];

export default function TU122() {
  return (
    <section className="relative w-full flex flex-col items-center py-10 mt-20">
      <h2 className="text-2xl md:text-4xl font-semibold text-[#545454] mb-25 text-center">
      Why Northwestern University ?
      </h2>

      {/* Line + Dots as One Image */}
      <div className="relative w-full max-w-[1000px] h-auto aspect-[1000/400]">
        <Image
          src="/images/tu2Img.png"
          alt="Timeline line"
          fill
          className="object-contain"
          priority
        />

        {/* Overlay Texts */}
        {points.map((point, index) => (
          <p
            key={index}
            className={`text-[#000000] text-xs sm:text-sm md:text-base ${point.style}`}
          >
            {point.text}
          </p>
        ))}
      </div>
    </section>
  );
}
